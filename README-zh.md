...
`luainstallers`是对既有旧项目[LuaToEXE](https://github.com/Water-Run/luaToEXE)的全面升级: 通过封装预编译的[luastatic](https://github.com/ers35/luastatic/tree/master)代替[srlua](https://github.com/LuaDist/srlua), 使打包后的可执行文件成为真正的原生可执行文件, 支持多平台, 也可执行编译扩展; 并实现了依赖分析引擎, 具备自动获取依赖列表的能力(包括查找从`luarocks`安装的库).  
